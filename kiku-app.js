...
const UPDATES=[
  {version:'3.13',date:'2026.06.24',items:['패치 버전 표기 방식 변경 — X.YY.Z 구조로 전면 개편 (X: 대규모 구조 개편 시 증가 / YY: 기능 업데이트마다 증가 / Z: 버그 수정마다 증가, YY 변경 시 0으로 초기화). 기존 v1.0~v3.11 전체 버전 번호를 새 규칙에 맞게 소급 재정리']},
  {version:'3.12',date:'2026.06.24',items:['이상형 월드컵 목록 카드의 썸네일 영역을 더 크게 키우고, 사진이 잘리지 않고 전체가 보이도록 수정','"시작하기" 버튼 텍스트가 줄바꿈되던 문제 수정 ("시작"으로 축약, 줄바꿈 방지 처리)']},
  {version:'3.11',date:'2026.06.24',items:['이상형 월드컵 플레이/최종 결과 화면의 팝업 크기를 크게 확대','사진이 정사각형이 아니어도 잘리지 않고 전체가 보이도록 수정 (빈 공간은 배경색으로 채움)']},
  {version:'3.10',date:'2026.06.24',items:['이상형 월드컵 라인업 등록 화면에서 "설명" 입력칸 제거 — 이름에 바로 설명을 적는 방식으로 통일 (예: "김치찌개를 끓이는 티라노사우르스")','라인업 등록 시 사진 첨부 / 이미지 링크 / 유튜브 링크 중 하나는 다시 필수로 변경 (이전 단계에서 선택사항으로 바꿨던 것을 되돌림)']},
  {version:'3.09',date:'2026.06.24',items:['이상형 월드컵 라인업 등록 시 사진/링크 없이 이름만으로도 등록 가능 (예: "김치찌개를 끓이는 티라노사우르스" 처럼 이름에 직접 설명을 적는 방식)','사진·영상이 없는 라인업은 목록·랭킹·플레이 화면에서 빈 이미지 아이콘으로 표시']},
  {version:'3.08',date:'2026.06.24',items:['이상형 월드컵 라인업 등록 시 "이미지 링크(URL)" 직접 입력 옵션 추가 — 사진 첨부 / 이미지 링크 / 유튜브 링크 중 하나 선택 가능']},
  {version:'3.07',date:'2026.06.24',items:['이상형 월드컵 제작 중 / 플레이 중에는 바탕 클릭이나 "취소"·"그만하기" 버튼을 눌러도 한 번 더 확인하도록 변경 — 실수로 화면이 꺼져서 작업 내용이 날아가는 것을 방지']},
  {version:'3.06',date:'2026.06.24',items:['이상형 월드컵 목록 화면에 "임시저장 확인" 버튼 추가 — 제목/설명/라인업 개수와 썸네일을 미리 보고 "이어서 작성" 또는 "삭제" 선택 가능']},
  {version:'3.05',date:'2026.06.24',items:['이상형 월드컵 카드에 "댓글" 버튼 추가 — 플레이 후 남겨진 우승 코멘트를 모아서 최신순으로 확인 가능','이상형 월드컵 제작 중 "임시저장" 기능 추가 — 계정별로 1개씩 저장되며, 다음에 "월드컵 만들기"를 눌렀을 때 이어서 작성할지 선택할 수 있음 (사진도 임시저장 시점에 미리 업로드되어 유지됨)','월드컵 만들기 완료 시 임시저장본은 자동으로 삭제됨']},
  {version:'3.04',date:'2026.06.24',items:['이상형 월드컵 플레이 기능 추가 — 강수 선택 후 랜덤 대진표 생성, 1대1로 클릭하며 진출하는 방식','라인업 개수가 선택한 강수에 딱 맞지 않으면 부전승을 자동 배정해 대진표를 맞춤','사진 라인업과 유튜브 영상 라인업 모두 동일하게 "선택" 버튼으로 통일 (영상은 큰 화면으로 그 자리에서 바로 재생)','최종 우승 결정 시 우승자를 사진과 함께 보여주고, 로그인 회원은 한줄 코멘트 작성 가능','랭킹 화면 추가 — 라인업별 최종 우승 횟수와 매치 승률을 함께 표시']},
  {version:'3.03',date:'2026.06.24',items:['놀이터 탭에 하위 탭 구조 추가 — "노래 토너먼트"(기존 이상형월드컵, 명칭 변경)와 "이상형 월드컵"(신규) 분리','이상형 월드컵 신설 — 회원 누구나 주제/설명/라인업(사진 또는 유튜브 링크)을 등록해 직접 만들 수 있음, 라인업 개수 제한 없음','이상형 월드컵 목록에 인기순/최신순 정렬 및 카드형 리스트(썸네일/제목/설명/참가 인원수) 추가, 삭제는 제작자 본인 또는 운영진만 가능','(플레이/랭킹 기능은 다음 업데이트에서 제공)']},
  {version:'3.02',date:'2026.06.23',items:['노래 추천 게시판·플레이리스트에 유튜브 링크 입력 추가, "유튜브에서 찾기" 버튼으로 검색 결과 새 탭 바로 열기','유튜브 링크 없는 곡은 이상형월드컵 후보 목록에서 자동 제외, 입력 화면에 안내 문구 표시','이상형월드컵 투표 화면에서 "들어보기" 버튼으로 곡을 그 자리에서 바로 재생 가능']},
  {version:'3.01.1',date:'2026.06.23',items:['[버그 수정] 노래 이상형월드컵 "시작" 버튼이 반응 없던 문제 수정 — Firestore가 지원하지 않는 중첩 배열 구조가 원인, 데이터 구조 변경 및 오류 발생 시 알림 표시 추가']},
  {version:'3.01',date:'2026.06.23',items:['"놀이터" 탭 신설 — 노래 이상형월드컵을 명예의 전당에서 분리해 독립 탭으로 이동','"리포트" 탭을 "통계" 탭에 통합 (월별/분기별 리포트는 통계 화면 하단에서 확인)']},
  {version:'2.09',date:'2026.06.23',items:['노래 이상형월드컵 추가 — 명예의 전당 탭에서 운영진이 추천곡으로 토너먼트 개설, 회원 투표로 라운드 진행, 우승곡은 역대 우승곡 명단에 영구 기록','롤링페이퍼 추가 — 회원 프로필에서 서로에게 메시지 남기기 (익명 가능), 본인/운영진만 삭제 가능']},
  {version:'2.08',date:'2026.06.23',items:['프로필에 "출석 도장판" 추가 — 전체 벙을 도장 형태로 표시, 참석/불참 한눈에 확인','프로필에 "같이 가장 많이 만난 멤버" TOP3 카드 추가','벙 카드에 "회고" 버튼 추가 — 참석 인원, 첫 참석자, 오랜만에 복귀한 멤버, 단골 멤버 등을 자동 정리해 카카오톡 공유용 텍스트로 생성']},
  {version:'2.07.1',date:'2026.06.23',items:['[버그 수정] 정산 계산기에서 금액 입력 시 한 글자만 쳐도 커서가 사라지던 문제 수정']},
  {version:'2.07',date:'2026.06.18',items:['모임비 정산 계산기 추가 — 사이드바 "정산" 탭, 항목별 금액·참가인원 입력 시 자동 분담 계산','벙 선택 모드(참석자 자동 연동) / 직접 입력 모드 둘 다 지원, 벙 카드에서도 바로 정산 진입 가능','정산 내역은 정산 탭에 저장되어 나중에 다시 확인 가능, 오픈채팅 송금용 복사 텍스트 자동 생성']},
  {version:'2.06',date:'2026.06.18',items:['게시판(자유게시판/건의사항) 글쓰기·수정 시 사진 최대 4장 첨부 가능','사진은 본문 원하는 위치에 삽입 가능 (커서 위치에 [이미지] 표시 자동 삽입, 자유롭게 이동 가능)']},
  {version:'2.05',date:'2026.06.18',items:['가로 스크롤 버그 근본 원인 수정 (레이아웃 구조 문제) + 긴 글은 일정 글자 수 이후 "더보기"로 처리','공지사항 작성자명도 연동된 프로필 이름을 우선 사용하도록 수정']},
  {version:'2.04',date:'2026.06.18',items:['회원에게 운영진/모임장 역할 부여 기능 추가 (회원 명단에서 지정, 동일 관리 권한)','오늘의 노래 추천 — 대시보드에 매일 자동 추천 (운영진 플레이리스트 + 노래 추천 게시판 추천곡 합산)','노래 추천 게시판 추가 — 곡명/아티스트/추천 이유 입력','게시판·공지·댓글에 긴 글(줄바꿈 없는 텍스트) 작성 시 페이지가 가로로 길게 늘어나던 버그 수정']},
  {version:'2.03',date:'2026.06.18',items:['모바일/사파리 로그인 오류 수정 (팝업 우선 방식 + 실패 시 원인 표시)','로그인 없이 둘러보기(게스트 모드) 추가','구글 로그인 시 개인정보 수집·이용 안내 동의 절차 추가','회원 프로필 "목록으로" 버튼 작동 오류 수정','프로필 사진이 전체 회원 목록·이달의 MVP·이달의 벙주 카드에도 반영되도록 수정','운영진 계정도 회원 프로필 연동 가능하도록 수정 (즉시 연결), 회원 명단 탭에 "내 프로필 연결" 버튼 추가']},
  {version:'2.02',date:'2026.06.18',items:['게시판 기능 추가 — 자유게시판, 건의사항(익명 가능), 댓글 기능','회원 프로필 ↔ 구글 계정 연결 시스템 (운영진 승인 방식)','프로필 커스텀 — 닉네임, 사진, 한줄소개, 최애곡/아티스트 (본인만 수정 가능)','통계 탭 기준을 최근 2개월 활동성으로 변경, 명예의 전당(전체 역대)과 역할 구분']},
  {version:'2.01',date:'2026.06.17',items:['Firebase 전환 — 실시간 동기화, 회원별 Google 로그인','공지사항 탭 추가 — 작성/수정/삭제, 상단 고정, 중요 표시','운영진/일반 회원 권한 분리','Firebase Storage로 갤러리 전환']},
  {version:'1.05',date:'2026.06.17',items:['캘린더 탭 추가','회원 프로필 탭 추가']},
  {version:'1.04',date:'2026.06.17',items:['명예의 전당 강화 — 스트릭 랭킹, 벙주 랭킹, 월별 MVP']},
  {version:'1.03',date:'2026.06.17',items:['대시보드 오른쪽 하단 — MVP, 신규 회원, 기념일, 업적 피드']},
  {version:'1.02',date:'2026.06.16',items:['대시보드 리디자인, 업적 시스템, 사이드바 레이아웃']},
  {version:'1.01',date:'2026.06.15',items:['최초 출시']},
];
